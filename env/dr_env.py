...
class DefeatRoachesEnv(gym.Env):
    def __init__(self, screen_size=64, step_mul=8, is_visualize=False, reselect_army_each_nstep=5, tensorboard_dir=None, tensorboard_prefix="rainbow"):
        ...
        self._env = sc2_env.SC2Env(
            ...
            agent_interface_format=features.AgentInterfaceFormat(
                feature_dimensions=features.Dimensions(screen=self.screen_size, minimap=self.screen_size),
        ...
        self.action_space = Discrete(self.screen_size ** 2) # https://github.com/pekaalto/sc2atari/blob/master/sc2/sc2toatari.py#L39
        self.observation_space = Box(
            low=0, high=features.SCREEN_FEATURES.player_relative.scale, shape=[self.screen_size, self.screen_size, 1]
        )
        ...
        if self.tensorboard_dir is not None:
            self.tb_path = os.path.join(os.path.abspath(tensorboard_dir), 
                f'{tensorboard_prefix}_{datetime.datetime.now().strftime("%y%m%d_%H%M")}')
            self.writer = SummaryWriter(self.tb_path)

    def reset(self):
        self.attack_action_id = [k for k in actions.FUNCTIONS if k.name == 'Attack_screen'][0].id
        self.steps = 0
    ...
    def step(self, action):
        ...
        if self.reselect_army_each_nstep > 0 and self.steps % self.reselect_army_each_nstep == 0:
            self._select_army()
    ...
    def _get_state(self):
        return self._pysc2_timestep.observation["feature_screen"][features.SCREEN_FEATURES.player_relative.index][..., np.newaxis]
    ...
    def _summarize_episode(self):
        ...
        print(f"episode {self.episode_counter}, score: {episode_score}, - avg {round(r.mean(), 3)}, min {round(r.min(), 3)}, max {round(r.max(),3)}")
        if self.tensorboard_dir is not None:
            self.writer.add_scalar('episodic_score', episode_score, self.episode_counter)
            self.writer.add_scalar('total_score', self.total_score, self.episode_counter)  
        sys.stdout.flush()
