    ...
    def summarize(self):
        ...
        if self.tensorboard_log_dir:
            ...
            if self.sliding_window[0] != -1:
                self.writer.add_scalar('mean_score', np.mean(self.sliding_window), self.episode)
                print(f"Finished episode {self.episode}, mean score {np.mean(self.sliding_window)}, list: {self.sliding_window}")
                self.sliding_window = [-1]*10
            # epsilon
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                if self.is_training:
                    ...
                    self.curr_epsilon = epsilon if epsilon != None else self.curr_epsilon
                # update variables
                state = next_state
                self.episodic_score += reward
            self.summarize()
    ...
    def run_sarsa(self, episodes):
        for _ in range(episodes):
            ...
            while not done:
                ...
                # update the agent with the new experience
                if self.is_training:
                    epsilon = self.agent.update(state, action, reward, next_state, next_action, done)
                    self.curr_epsilon = epsilon if epsilon != None else self.curr_epsilon
                # update variables
                state = next_state
                action = next_action
